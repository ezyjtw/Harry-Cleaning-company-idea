...
export default function AnalyticsPage() {
  ...
  if (!data) {
    return (
      <div className="p-8 text-center text-sm text-gray-500">No analytics data available yet.</div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <StatCard
          label="Avg Rating"
          value={`${q.avgRating.toFixed(1)}/5`}
          sub={`${q.totalReviews} reviews`}
          color="amber"
        />
        <StatCard
          label="Repeat Rate"
          value={`${c.repeatRate}%`}
          sub={`${c.repeatCustomers} returning`}
          color="purple"
        />
        <StatCard
          label="Refund Rate"
          value={`${f.refundRate}%`}
          sub={formatCurrency(f.refundTotal)}
          color={f.refundRate > 5 ? 'red' : 'gray'}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <SectionCard title="Revenue Trend" subtitle="Monthly completed booking revenue">
          <BarChart
            data={data.revenueTrends.map((t) => ({
              label: t.period.split('-')[1],
              value: t.revenue,
            }))}
            maxValue={maxRev}
          />
              ...
              <div key={s.serviceType}>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <span className="text-[10px] text-gray-400">
                    avg {formatCurrency(s.avgValue)}/booking
                  </span>
                </div>
              </div>
            ))}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <StatCard
          ...
          value={formatCurrency(f.totalCleanerEarnings)}
          color="blue"
        />
        <StatCard
          label="Platform Fees"
          value={formatCurrency(f.totalPlatformFees)}
          sub={`${f.platformMarginPercent}% margin`}
          color="purple"
        />
        <StatCard
          label="Revenue / Member"
          value={formatCurrency(f.revenuePerTeamMember)}
          color="teal"
        />
        <StatCard
          label="Avg Booking Value"
          value={formatCurrency(f.avgBookingValue)}
          color="amber"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard
          ...
          subtitle="Monthly revenue, cleaner earnings, and platform fees"
        >
          <BarChart
            data={data.revenueTrends.map((t) => ({
              label: t.period.split('-')[1],
              ...
              <div key={t.period} className="flex justify-between text-xs">
                ...
                <div className="flex gap-4">
                  ...
                  <span className="text-blue-600">
                    {formatCurrency(t.cleanerEarnings)} paid out
                  </span>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                ...
                <p className="text-2xl font-bold text-green-800 mt-1">
                  {formatCurrency(f.addonRevenue)}
                </p>
                <p className="text-xs text-green-600 mt-1">
                  {f.addonAttachmentRate}% attachment rate
                </p>
              </div>
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          label="Total Jobs"
          value={String(team.reduce((s, t) => s + t.completedJobs, 0))}
          color="green"
        />
        <StatCard
          label="Avg Rating"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-right py-2 px-3 text-xs font-medium text-gray-500 uppercase hidden md:table-cell">
                  Avg Duration
                </th>
                <th className="text-right py-2 px-3 text-xs font-medium text-gray-500 uppercase hidden md:table-cell">
                  Cancel %
              ...
              {team.map((m, i) => (
                <tr key={m.userId} className="hover:bg-gray-50">
                  ...
                  <td className="text-right py-3 px-3">
                    <span
                      className={`text-sm font-medium ${m.avgRating >= 4.5 ? 'text-green-600' : m.avgRating >= 4.0 ? 'text-amber-600' : 'text-red-600'}`}
                    >
                  ...
                  <td className="text-right py-3 px-3 hidden md:table-cell">
                    <span
                      className={`text-sm ${m.cancellationRate > 10 ? 'text-red-600 font-medium' : 'text-gray-600'}`}
                    >
                      {m.cancellationRate}%
                    </span>
                  </td>
                  <td className="text-right py-3 px-3 text-sm text-gray-600 hidden lg:table-cell">
                    {m.ratingBreakdown.thoroughness > 0
                      ? m.ratingBreakdown.thoroughness.toFixed(1)
                      : '-'}
                  </td>
                  <td className="text-right py-3 px-3 text-sm text-gray-600 hidden lg:table-cell">
                    {m.ratingBreakdown.punctuality > 0
                      ? m.ratingBreakdown.punctuality.toFixed(1)
                      : '-'}
                  </td>
                  <td className="text-right py-3 px-3 text-sm text-gray-600 hidden lg:table-cell">
                    {m.ratingBreakdown.communication > 0
                      ? m.ratingBreakdown.communication.toFixed(1)
                      : '-'}
                  </td>
                </tr>
              ))}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <StatCard
          label="Repeat Rate"
          value={`${c.repeatRate}%`}
          color={c.repeatRate >= 50 ? 'green' : 'amber'}
        />
        <StatCard
          label="Avg Bookings / Customer"
          value={c.avgBookingsPerCustomer.toFixed(1)}
          color="purple"
        />
        <StatCard
          label="Avg Spend / Customer"
          value={formatCurrency(c.avgSpendPerCustomer)}
          color="teal"
        />
        <StatCard
          label="Avg Spend / Booking"
          value={formatCurrency(c.avgSpendPerBooking)}
          color="amber"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <SectionCard
          ...
          subtitle="Spend distribution across your customer base"
        >
          <div className="space-y-3">
            {c.topCustomers.map((cust, i) => (
              <HorizontalBar
                key={cust.name + i}
                label={cust.name}
                value={Math.round(cust.totalSpent)}
                max={maxSpent}
                color="green"
              />
            ))}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <StatCard
          ...
          color={o.completionRate >= 90 ? 'green' : 'amber'}
        />
        <StatCard
          label="Cancellation Rate"
          value={`${o.cancellationRate}%`}
          color={o.cancellationRate <= 5 ? 'green' : o.cancellationRate <= 15 ? 'amber' : 'red'}
        />
        <StatCard
          label="On-Time Rate"
          value={`${o.onTimeRate}%`}
          color={o.onTimeRate >= 90 ? 'green' : 'amber'}
        />
        <StatCard
          label="Avg Response"
          value={`${o.avgResponseTimeMinutes.toFixed(0)}min`}
          sub="booking to acceptance"
          color="blue"
        />
        <StatCard
          label="Avg Completion"
          value={`${o.avgCompletionTimeHours.toFixed(1)}h`}
          sub="acceptance to done"
          color="purple"
        />
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <StatCard
          label="Thoroughness"
          value={q.avgThoroughness > 0 ? `${q.avgThoroughness.toFixed(1)}/5` : 'N/A'}
          color="green"
        />
        <StatCard
          label="Punctuality"
          value={q.avgPunctuality > 0 ? `${q.avgPunctuality.toFixed(1)}/5` : 'N/A'}
          color="blue"
        />
        <StatCard
          label="Communication"
          value={q.avgCommunication > 0 ? `${q.avgCommunication.toFixed(1)}/5` : 'N/A'}
          color="purple"
        />
        <StatCard
          label="Dispute Rate"
          value={`${q.disputeRate}%`}
          color={q.disputeRate <= 2 ? 'green' : q.disputeRate <= 5 ? 'amber' : 'red'}
        />
      </div>
