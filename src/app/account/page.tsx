  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([userData, addressData]) => {
        if (userData) {
          const p = { name: userData.name || '', email: userData.email || '', phone: userData.phone || '' };
          setProfile(p);
          setProfileForm(p);
        }
  ...
  return (
    <div className="space-y-6">
            ...
            {addresses.map((addr) => (
              ...
              >
                ...
                <div className="flex shrink-0 gap-2">
                  {!addr.isDefault && (
                    <button onClick={() => handleSetDefaultAddress(addr.id)} className="text-xs text-gray-500 hover:text-gray-700">
                      Set default
                    </button>
                  )}
                  <button onClick={() => handleRemoveAddress(addr.id)} className="text-xs text-red-500 hover:text-red-700">
                    Remove
                  </button>
                </div>
              </li>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        {passwordSuccess && (
          <div className="mt-3 rounded-lg bg-green-50 p-3 text-sm text-green-700">Password changed successfully.</div>
        )}
        {passwordError && (
