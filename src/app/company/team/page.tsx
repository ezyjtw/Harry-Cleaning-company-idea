      ...
      .then((data) => {
        setMembers(
          (data.members || []).map((m: Record<string, unknown>) => ({
            ...
            name: ((m as Record<string, unknown>).user as Record<string, unknown>)?.name || 'Unknown',
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Email</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredMembers.map((member) => (
                <tr key={member.userId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    {member.role === 'owner' ? (
                      <span className={`text-xs font-medium rounded-full px-2.5 py-1 ${roleStyles.owner}`}>
                        Owner
                      </span>
                    ) : (
                      <select
