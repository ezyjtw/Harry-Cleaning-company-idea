  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-ink-3 mt-1">Configure platform-wide settings and pricing</p>
      </div>

      <div className="space-y-6">
        {/* Commission rate */}
        <div className="bg-surface rounded-xl border border-line p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">Commission Rate</h2>
          <div className="flex items-center gap-3">
            <input
              ...
              min="0"
              max="50"
              className="w-24 rounded-lg border border-line px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
            />
            <span className="text-sm text-ink-3">% platform commission on each booking</span>
          </div>
        ...
        {/* Service types */}
        <div className="bg-surface rounded-xl border border-line p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">Service Types</h2>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                      service.active ? 'bg-trust' : 'bg-line'
                    }`}
                  >
                    <span
                  ...
                  <span
                    className={`text-sm font-medium ${service.active ? 'text-ink' : 'text-ink-3'}`}
                  >
                    {service.name}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm text-ink-3">£{service.basePrice}/hr base</span>
                  <button
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-line px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
            />
            <input
              type="number"
              placeholder="£/hr"
              ...
              className="w-24 rounded-lg border border-line px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
            />
            <button
        ...
        {/* Pricing rules */}
        <div className="bg-surface rounded-xl border border-line p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">Pricing Rules</h2>
            ...
            {pricingRules.map((rule) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-sm font-medium ${rule.active ? 'text-ink' : 'text-ink-3'}`}
                  >
                    {rule.name}
                  </span>
                </div>
                <span className="text-sm text-ink-3">{rule.multiplier}x multiplier</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-surface rounded-xl border border-line p-6">
          <h2 className="text-lg font-semibold text-ink mb-4">Service Areas</h2>
          <div className="space-y-3">
            {areas.map((area) => (
              <div
                key={area.id}
                className="flex items-start justify-between py-3 border-b border-line last:border-0"
              >
                <div className="flex items-start gap-3">
                  <button
                    onClick={() => toggleArea(area.id)}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors mt-0.5 flex-shrink-0 ${
                      area.active ? 'bg-trust' : 'bg-line'
                    }`}
                  >
                    <span
                      className={`inline-block h-3.5 w-3.5 transform rounded-full bg-surface transition-transform ${
                  ...
                  <div>
                    <span
                      className={`text-sm font-medium ${area.active ? 'text-ink' : 'text-ink-3'}`}
                    >
                      {area.name}
                    </span>
                    <p className="text-xs text-ink-3 mt-0.5">{area.postcodes}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          {saved && (
            <span className="text-sm text-trust font-medium flex items-center gap-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          <button
            onClick={handleSave}
            className="px-8 py-2.5 bg-primary text-white font-medium rounded-lg hover:bg-primary-hover transition-colors"
          >
            Save Settings
          </button>
        </div>
