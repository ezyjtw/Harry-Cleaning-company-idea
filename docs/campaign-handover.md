...
- **NSCameraUsageDescription** — the proper camera fix, ledgered NOT built:
  ...
  build — SCHEDULE MOVED UP (James, 9 Sep): the 1.0.1 build happens when
  P3's dust settles rather than waiting for an unrelated native need; the
  build order itself still comes with his word. Version-bump law applies
  (1.0.0 → 1.0.1). Un-gate the in-shell
