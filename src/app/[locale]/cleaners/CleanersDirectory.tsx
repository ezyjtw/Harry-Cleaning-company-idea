...
// Raw /api/cleaners card shape → the page's Cleaner type. Shared by the
// client fetch AND the server-rendered seed so the two can never drift.
function mapApiCleaners(raw: Record<string, unknown>[]): Cleaner[] {
  return raw.map((c) => ({
    id: c.id as string,
...
export default function CleanersDirectory({
  initialCleaners,
}: {
  initialCleaners: Record<string, unknown>[] | null;
}) {
  return (
    <Suspense
      fallback={
    ...
    >
      <CleanersContent initialCleaners={initialCleaners} />
    </Suspense>
  );
}
...
function CleanersContent({
  ...
  // Server-rendered seed (crawler-visible list) hydrates the unfiltered view;
  // a postcode in the URL still goes through the client fetch as before.
  const seeded = Boolean(initialCleaners && !searchParams.get('postcode'));
  const [allCleaners, setAllCleaners] = useState<Cleaner[]>(
    seeded ? mapApiCleaners(initialCleaners as Record<string, unknown>[]) : []
  );
  ...
  useEffect(() => {
    if (skipFirstFetch.current) {
      // First render already has the server-rendered list.
