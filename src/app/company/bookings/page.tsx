  ...
  const handleAssign = async (bookingFullId: string, cleanerId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBookings((prev) =>
          prev.map((b) =>
            b.fullId === bookingFullId
              ? { ...b, assignedTo: assigned?.name || '', cleanerId }
              : b
          )
        );
      }
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex flex-wrap gap-2">
            {[
              ...
              >
                {label}
                {key !== 'all' && statusCounts[key] ? ` (${statusCounts[key]})` : key === 'all' ? ` (${bookings.length})` : ''}
              </button>
            ))}
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Address</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredBookings.map((booking) => (
                <tr key={booking.fullId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-gray-900">&pound;{booking.amount}</td>
                  <td className="px-6 py-4">
                    ...
                    {booking.status !== 'completed' && booking.status !== 'cancelled' ? (
                      <select
                        ...
                        className={`text-sm rounded-lg px-2 py-1.5 border focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                          booking.cleanerId ? 'border-gray-300 text-gray-700' : 'border-red-300 text-red-500'
                        }`}
                      >
