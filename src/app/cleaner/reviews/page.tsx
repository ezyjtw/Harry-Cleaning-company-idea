...
function TestimonialCard({
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => onChange({ ...t, image: reader.result as string });
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-shrink-0">
          ...
          <button
            ...
            className="w-16 h-16 rounded-lg bg-white overflow-hidden flex items-center justify-center transition hover:ring-2 hover:ring-gold/30"
          ...
          >
            {t.image ? (
              ...
              <div className="text-center">
                <svg
                  className="w-5 h-5 text-ink-3/30 mx-auto"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span className="font-jost text-[9px] text-ink-3/40 mt-0.5 block">Photo</span>
              </div>
            )}
          </button>
          {t.image && (
            <button
              ...
              className="mt-1 w-full text-center font-jost text-[10px] text-ink-3/40 hover:text-red-400 transition-colors"
            >
              Remove
            </button>
          )}
...
export default function ReviewsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            {testimonials.length > 0 && (
              <button
                ...
                className="rounded-full px-5 py-1.5 bg-ink text-cream font-jost text-[12px] font-light hover:bg-ink/90 transition disabled:opacity-50"
              >
      ...
      {/* Saved testimonials shown as review cards */}
      {testimonials.filter((t) => t.clientName.trim() && t.text.trim()).length > 0 && (
        <div className="mt-10">
          <h2 className="font-cormorant text-xl font-light text-ink mb-1">Your Testimonials</h2>
          <p className="font-jost text-sm font-light text-ink-3 mb-5">
            These are shown on your public profile
          </p>
          <div className="space-y-4">
            {testimonials
              ...
              .map((t, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <p className="font-jost text-sm text-ink">{t.clientName}</p>
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg
                            key={star}
                            className={`w-3.5 h-3.5 ${star <= t.rating ? 'text-gold' : 'text-ink-3/15'}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    ...
                    <p className="font-jost text-sm font-light text-ink-2">{t.text}</p>
                    <p className="font-jost text-[11px] text-ink-3/50 mt-2">Client testimonial</p>
                  </div>
                </div>
              ))}
          </div>
