        ...
        >
          ...
          <div className="flex items-center justify-center gap-1 mt-2">
            {Array.from({ length: 5 }).map((_, i) => (
              <svg
                key={i}
                className={`w-5 h-5 ${i < Math.round(Number(overallRating)) ? 'text-gold' : 'text-ink-3/20'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        {ratingFilters.map((r) => (
          <button
            key={r}
            onClick={() => setFilter(r)}
            className={`px-3 py-1.5 font-jost text-sm font-light transition-colors ${
              filter === r ? 'bg-ink text-cream' : 'bg-cream-2 text-ink-3 hover:text-ink'
            }`}
            style={{ border: filter === r ? 'none' : '0.5px solid rgba(14,14,12,0.1)' }}
          >
            {r === 0 ? 'All' : `${r} Star`}
          </button>
        ))}
              ...
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <svg
                      key={i}
                      className={`w-4 h-4 ${i < review.rating ? 'text-gold' : 'text-ink-3/20'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              ...
              {!review.reply && replyingTo === review.id ? (
                <div className="mt-3">
                  <textarea
                    ...
                    className="w-full px-3 py-2 font-jost text-sm font-light text-ink bg-cream focus:outline-none focus:ring-1 focus:ring-gold"
                    style={{ border: '0.5px solid rgba(14,14,12,0.1)' }}
                    rows={3}
                  />
                  <div className="flex gap-2 mt-2">
                    <button
                      ...
                      className="px-4 py-1.5 bg-ink text-cream font-jost text-sm font-light hover:bg-ink/90 transition-colors"
                    >
                      Submit Reply
                    </button>
                    <button
                      ...
                      className="px-4 py-1.5 bg-cream text-ink font-jost text-sm font-light hover:bg-cream-2 transition-colors"
                      style={{ border: '0.5px solid rgba(14,14,12,0.1)' }}
                    >
                      Cancel
                    </button>
