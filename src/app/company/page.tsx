...
export default function CompanyDashboard() {
      ...
      .then(([dashData, bookingsData, analyticsData]) => {
        ...
        setRecentBookings(
          (bookingsData.bookings || []).map((b: Record<string, unknown>) => ({
            ...
            customer: (b.client as Record<string, unknown>)?.name || (b.guestName as string) || 'Guest',
  ...
  const completionRate = dashboard && dashboard.totalBookings > 0
    ? Math.round((dashboard.completedBookings / dashboard.totalBookings) * 100)
    : 0;

  const stats = [
    { label: 'Total Bookings', value: String(dashboard?.totalBookings || 0), change: `${dashboard?.pendingBookings || 0} pending`, color: 'blue' },
    { label: 'Revenue', value: `\u00a3${(dashboard?.totalRevenue || 0).toLocaleString()}`, change: `${dashboard?.completedBookings || 0} completed`, color: 'green' },
    { label: 'Team Size', value: String(dashboard?.totalTeamMembers || 0), change: `${dashboard?.activeTeamMembers || 0} active`, color: 'purple' },
    { label: 'Avg Rating', value: (dashboard?.averageRating || 0).toFixed(1), change: `Based on ${dashboard?.totalReviews || 0} reviews`, color: 'yellow' },
    { label: 'Completion Rate', value: `${completionRate}%`, change: `${dashboard?.cancelledBookings || 0} cancelled`, color: 'teal' },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <a href="/company/bookings" className="text-sm text-green-600 hover:text-green-800 font-medium">
              View All
            </a>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Assigned To</th>
                  ...
                  recentBookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-3">
                        ...
                        <p className="text-xs text-gray-400">{booking.id} &middot; {booking.service}</p>
                      </td>
                      <td className="px-6 py-3 hidden sm:table-cell">
                        <span className={`text-sm ${booking.assignedTo === 'Unassigned' ? 'text-red-500 font-medium' : 'text-gray-600'}`}>
                          {booking.assignedTo}
                        </span>
                      </td>
                      <td className="px-6 py-3 text-sm font-medium text-gray-900">&pound;{booking.amount}</td>
                      <td className="px-6 py-3">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900">&pound;{(dashboard?.totalRevenue || 0).toLocaleString()}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              ...
              <a href="/company/team" className="text-sm text-green-600 hover:text-green-800 font-medium">
                View Team
              </a>
            </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {performer.completedJobs} jobs &middot; {performer.rating.toFixed(1)} rating
                        </p>
                      </div>
                    </div>
                  </div>
